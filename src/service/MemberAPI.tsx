...
import {ApiEnums, HeaderKeys, HTTP_METHOD, TOKEN_EXPIRES} from '../utils/api/ServerEnum.tsx';
...
export const login = async (request: JoinRequestDTO): Promise<number> => {
    try {
        ...
        // 서버 토큰 (만료시간 - 1분) = 9분  1분 전에 토큰 재발급
        setTimeout(tokenRefresh, TOKEN_EXPIRES.access - 1000 * 60);
        return response.status;
    } catch (error) {
